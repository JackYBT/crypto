...
function computeInput(depth, transcript, nullifier) {
    // TODO
    result = {};
    let temp = new SparseMerkleTree(depth);
    let nonce;
    for (var i = 0; i < transcript.length; ++i) {
      if (transcript[i].length == 1){ 
        temp.insert(transcript[i][0]);
      } else {
        if (transcript[i][0] == nullifier){
          nonce = transcript[i][1];
        }
        temp.insert(mimc2(transcript[i][0], transcript[i][1]));
      }
    }
    result["digest"] = temp.digest;
    result["nullifier"] = nullifier;
    result["nonce"] = nonce;
    var path = temp.path(mimc2(nullifier,nonce));
    for (var j = 0; j < path.length; ++j){
      result["sibling[" + j.toString() + "]"] = path[j][0];
      result["direction[" + j.toString() + "]"] = Number(path[j][1]).toString();
    }
    return result;
}
