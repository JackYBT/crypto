...
const token_name = 'RamonToken';             // TODO: replace with name of your token
const token_symbol = 'RAM';               // TODO: replace with symbol for your token
...
const token_address = '0x2445f1D67F6fc33B99Da92cc6994A60Be2B59CB8';                   
const token_abi = [
	{
		"inputs": [],
...
const exchange_address = token_address; //'0xF686E64B01FF6F08D7eaE519901E39B615F76cA8';    
const exchange_abi = token_abi;            
...
async function calcMinMaxExhangeRate(maxSlippagePct) {
    var poolState = await getPoolState();
    return {
        max : ~~(100000000 * poolState['token_eth_rate'] * (100 + maxSlippagePct) / 100),
        min : ~~(100000000 * poolState['token_eth_rate'] * (100 - maxSlippagePct) / 100),
    };
}
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    var rates = await calcMinMaxExhangeRate(maxSlippagePct);
    var amountToken = ~~(amountEth * rates['max'] / 100000000);
    await token_contract.methods.approve(exchange_address, amountToken).send({from: web3.eth.defaultAccount, gas : 999999});
    // var allowance = await token_contract.methods.allowance(web3.eth.defaultAccount,exchange_address).call();
    await exchange_contract.methods.addLiquidity(rates['max'], rates['min']).send({from: web3.eth.defaultAccount, value: amountEth, gas : 999999});
}
...
async function removeLiquidity(amountEth, maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    var rates = await calcMinMaxExhangeRate(maxSlippagePct);
    await exchange_contract.methods.removeLiquidity(amountEth, rates['max'], rates['min']).send({from: web3.eth.defaultAccount, gas : 999999});
}

async function removeAllLiquidity(maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    var rates = await calcMinMaxExhangeRate(maxSlippagePct);
    await exchange_contract.methods.removeAllLiquidity(rates['max'], rates['min']).send({from: web3.eth.defaultAccount, gas : 999999});
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    // var poolState = await getPoolState();
    // var amountEthMin = poolState['eth_reserves'] * amountToken / (poolState['token_reserves'] + amountToken);
    // amountEthMin = ~~(amountEthMin * (100-maxSlippagePct));
    var rates = await calcMinMaxExhangeRate(maxSlippagePct);
    await token_contract.methods.approve(exchange_address, amountToken).send({from: web3.eth.defaultAccount, gas : 999999});
    // max is max eth to token price we accept
    await exchange_contract.methods.swapTokensForETH(amountToken, rates['max']).send({from: web3.eth.defaultAccount, gas : 999999});
}

async function swapETHForTokens(amountETH, maxSlippagePct) {
    /** TODO: ADD YOUR CODE HERE **/
    // var poolState = await getPoolState();
    // var amountTokensMin = poolState['token_reserves'] * amountETH / (poolState['eth_reserves'] + amountETH);
    // amountTokensMin = ~~(amountTokensMin * (100-maxSlippagePct));
    var rates = await calcMinMaxExhangeRate(maxSlippagePct);
    
    // min is the min eth to token price we accept
    await exchange_contract.methods.swapETHForTokens(rates['min']).send({from: web3.eth.defaultAccount, value: amountETH, gas : 999999});
}
